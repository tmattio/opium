...
module Params = struct
  ...
  let post_params_exn req body =
    let headers = req.Rock.Request.headers in
  ...
  let of_req_exn req body =
    let get_params = req.Rock.Request.target |> Uri.of_string |> of_uri_exn in
...
let ( >>= ) = Lwt.( >>= )
let respond_string ~status ~body () = Lwt.return (Rock.Response.of_string ~status body)
...
let execute_request schema ctx req =
  Rock.Body.to_string req.Rock.Request.body
  >>= fun body_string ->
...
let make_handler ?(make_context = fun _req -> ()) schema req =
  ...
  | `GET ->
    if Httpaf.Headers.get req.Rock.Request.headers "Connection" = Some "Upgrade"
       && Httpaf.Headers.get req.Rock.Request.headers "Upgrade" = Some "websocket"
    then
      (* TODO: Add subscription support when there is a good solution for websockets with Httpaf *)
      respond_string
...
let graphiql_handler ~api req =
  let accept_html =
    match Httpaf.Headers.get req.Rock.Request.headers "accept" with
    | None -> false
    | Some s -> List.mem "text/html" (String.split_on_char ',' s)
  in
  if accept_html
  then
    Static.serve "graphiql.html" req ~read:(fun () ->
        match Assets.read "graphiql.html" with
        | None -> Lwt.return_none
        | Some body ->
          let regexp = Str.regexp_string "%%GRAPHQL_API%%" in
          let body = Str.global_replace regexp api body in
          Lwt.return @@ Some (Rock.Body.of_string body))
