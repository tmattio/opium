...
let response_check = Alcotest.of_pp Rock.Response.pp_hum

let check_body actual expected =
  Rock.Body.to_string actual >|= fun body -> Alcotest.(check string) "body" body expected
;;

let default_uri = "/"
let json_content_type = Rock.Headers.of_list [ "Content-Type", "application/json" ]
let graphql_content_type = Rock.Headers.of_list [ "Content-Type", "application/graphql" ]
...
let suite =
  [ ( "POST with empty body"
    ...
    , fun () ->
        test_case
          ~req:(Rock.Request.make default_uri `POST ())
          ~rsp:(Rock.Response.of_string ~status:`Bad_request "Must provide query string")
    )
  ; ( "POST with json body"
    , `Quick
    , fun () ->
        ...
        in
        test_case
          ~req:(Rock.Request.make ~headers:json_content_type ~body default_uri `POST ())
          ~rsp:(Rock.Response.of_string ~status:`OK default_response_body) )
  ; ( "POST with graphql body"
    , `Quick
    , fun () ->
        let body = Rock.Body.of_string "{ hello }" in
        test_case
          ~req:
            (Rock.Request.make ~headers:graphql_content_type ~body default_uri `POST ())
          ~rsp:(Rock.Response.of_string ~status:`OK default_response_body) )
  ; ( "GET with empty query string"
    , `Quick
    , fun () ->
        test_case
          ~req:(Rock.Request.make default_uri `GET ())
          ~rsp:(Rock.Response.of_string ~status:`Bad_request "Must provide query string")
    )
  ; ( "GET with query"
    , `Quick
    , fun () ->
        ...
        test_case
          ~req:(Rock.Request.make (Uri.to_string uri) `GET ())
          ~rsp:(Rock.Response.of_string ~status:`OK default_response_body) )
